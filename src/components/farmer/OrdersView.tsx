  ...
  return (
    <div className="space-y-6 animate-in fade-in-50 duration-300">
      ...
      <div className="bg-white p-6 rounded-3xl border border-[#EEF3E8] shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {/* Status Filter & QR Verify Action */}
        <div className="flex items-center gap-2 flex-wrap">
          <button
            onClick={() => setIsQRVerifyOpen(true)}
            className="px-3.5 py-2 rounded-xl bg-[#245C3A] hover:bg-[#1E4D31] text-white text-xs font-bold flex items-center gap-1.5 transition-colors shadow-2xs"
          >
            <QrCode className="w-3.5 h-3.5" />
            <span>{currentLanguage === 'hi' ? 'क्यूआर सत्यापित करें' : 'Verify QR'}</span>
          </button>
                ...
                <div className="flex items-center gap-2">
                  {order.status === 'Payment Completed' && (
                    <button
                      ...
                      className="px-3.5 py-1.5 rounded-xl bg-amber-50 hover:bg-amber-100 border border-amber-200 text-amber-900 text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
                    >
                      <Star className="w-3.5 h-3.5 text-amber-600 fill-amber-500" />
                      <span>{currentLanguage === 'hi' ? 'खरीदार को रेटिंग दें' : 'Rate Buyer'}</span>
                    </button>
                  )}
                  <button
      ...
      {/* Transaction Rating Modal */}
      {ratingOrder && (
        <TransactionRatingModal
          isOpen={!!ratingOrder}
          onClose={() => setRatingOrder(null)}
          currentLanguage={currentLanguage}
          counterpartName={ratingOrder.buyerName}
          counterpartRole="buyer"
          orderId={ratingOrder.orderNumber}
