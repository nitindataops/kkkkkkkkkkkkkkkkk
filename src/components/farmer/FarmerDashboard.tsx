...
import { saveCropsToOfflineCache, getCachedCrops } from '../../services/offlineSyncService';
import { CropValuePreservationEngine } from './CropValuePreservationEngine';
    ...
    async function loadPersistedCrops() {
      ...
      try {
        ...
        if (res.success && Array.isArray(res.crops)) {
          ...
          setCrops(approved);
          saveCropsToOfflineCache(approved);
        } else {
          const cached = getCachedCrops();
          if (cached && cached.length > 0) {
            setCrops(cached as CropListing[]);
          } else {
            setCrops([]);
          }
        }
      } catch (err) {
        console.warn('Could not fetch persisted farmer crops, restoring offline cache:', err);
        const cached = getCachedCrops();
        if (cached && cached.length > 0) {
          setCrops(cached as CropListing[]);
        }
