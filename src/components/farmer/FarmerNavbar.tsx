        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 sm:h-18 gap-2 sm:gap-4">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              {/* ITEM 2B: 🌾 Value Engine */}
              <button
                id="farmer-nav-value-engine-btn"
                onClick={() => onSelectTab('value-engine')}
                className={`px-2.5 sm:px-3 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer border ${
                  currentTab === 'value-engine'
                    ? 'bg-[#245C3A] text-white border-[#245C3A] shadow-xs'
                    : 'bg-[#FBFAF4] hover:bg-[#EEF3E8] text-[#245C3A] border-[#EEF3E8]'
                }`}
                title={isHi ? 'फसल मूल्य संरक्षण इंजन' : 'Value Engine'}
              >
                <Sparkles className="w-4 h-4 text-[#5F8F45]" />
                <span className="hidden lg:inline">{isHi ? 'मूल्य संरक्षण' : 'Value Engine'}</span>
              </button>

              {/* ITEM 2C: 🔄 Offline Sync Status Badge */}
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              ...
              {/* 🌾 मूल्य संरक्षण इंजन (Crop Value Preservation Engine) */}
              <button
                id="menu-item-value-engine"
                onClick={() => handleMenuSelect('value-engine')}
                ...
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    <div className="font-bold text-sm text-[#26332B] group-hover:text-[#245C3A] transition-colors">
                      {isHi ? '🌾 मूल्य संरक्षण इंजन (Crop Value Engine)' : '🌾 Crop Value Preservation Engine'}
                    </div>
                    <div className="text-[11px] text-[#68736B]">
                      {isHi ? 'सेंस • प्रेडिक्ट • ऑप्टिमाइज़ • एक्ट • ट्रस्ट • कनेक्ट' : 'Sense • Predict • Optimize • Act • Trust • Connect'}
                    </div>
                  </div>
                </div>
