    ...
    <div className="fixed inset-0 z-50 bg-black/65 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
        ...
        <div className="p-4 sm:p-5 border-t border-[#EEF3E8] bg-white flex items-center justify-between gap-3 shrink-0">
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3.5 py-2.5 rounded-xl border border-[#245C3A]/30 text-xs font-bold text-[#245C3A] bg-[#245C3A]/5 hover:bg-[#245C3A]/10 flex items-center gap-1.5 transition-colors cursor-pointer"
            >
              <QrCode className="w-3.5 h-3.5 text-[#245C3A]" />
              <span>{isHi ? 'डिजिटल क्यूआर पासपोर्ट' : 'Digital QR Passport'}</span>
            </button>
      ...
      {isQRModalOpen && (
        <CropQRPassportModal
          ...
          passportData={{
            batchId: `KS-LOT-${crop.id || '2026-089'}`,
            cropName: crop.name || crop.cropName || 'Crop',
            variety: crop.variety,
            quantityKg: crop.quantityKg || 1000,
            grade: (crop.grade === 'PREMIUM' || crop.grade === 'STANDARD') ? crop.grade : 'STANDARD',
            qualityScore: crop.qualityScore || 90,
            farmerId: crop.farmerId || 'KISAN-UP-2026-8842',
            farmerName: (crop as any).farmerName || 'Rameshwar Patidar',
            district: crop.district || 'Bareilly',
            state: crop.state || 'Uttar Pradesh',
            harvestDate: crop.harvestedDate || crop.harvestDate || new Date().toISOString().split('T')[0],
            mandiBenchmarkRate: crop.currentMandiPrice || 2750,
