...
export function getValidGeminiModel(candidate?: string): string {
  if (
    candidate &&
    candidate.startsWith('gemini-') &&
    !candidate.includes('1.5') &&
    !candidate.includes('2.0') &&
    !candidate.includes('2.5') &&
    !candidate.includes('3.6')
  ) {
    return candidate;
  }
...
async function generateGeminiContent(
  ...
  const primaryModel = getValidGeminiModel(params.model);
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    try {
      ...
      console.warn(
        `[KisanSetu AI] Gemini ${model} failed (Status: ${status || 'unknown'}). Trying next fallback model...`
      );

      // If transient error (429, 503, 408) on first attempt, retry once after short delay
      if (i === 0 && (status === 429 || status === 503 || status === 408)) {
        await sleep(800);
        try {
          ...
          console.log(`[KisanSetu AI] Gemini retry success: ${model}`);
          return {
...
export async function processAiSupportChat(params: {
  ...
  // 6. Construct unified system instructions and attempt Fast Groq LLM first, falling back to Gemini
...
- Kisan Saathi also provides crop value preservation, quality grading, mandi intelligence, order tracking, and offline support.
...
`;
  ...
  // 6A. Try calling Groq API if GROQ_API_KEY is configured
  const groqApiKey = process.env.GROQ_API_KEY?.trim();
  if (groqApiKey) {
    try {
      const configuredGroq = process.env.GROQ_MODEL?.trim();
      ...
      groqMessages.push({
        role: 'user',
        content: `User Query:\n${sanitized}\n\nTarget Language: ${targetLang}${diagnostic.explanation ? `\n\nVerified Website Diagnostic:\n${diagnostic.explanation}` : ''}`,
      });

      for (const groqModel of groqCandidates) {
        try {
          console.log(`[Kisan Saathi AI] Calling Groq fast LLM: ${groqModel}`);
          ...
          if (groqRes.ok) {
            ...
            if (groqReply) {
              console.log(`[Kisan Saathi AI] Fast response from Groq (${groqModel}) received successfully`);
              return {
                replyText: groqReply,
                suggestedActions: diagnostic.suggestedActions,
                needsResolutionConfirmation: true,
            ...
            console.warn(`[Groq AI] Response error on ${groqModel} (${groqRes.status}): ${errText.slice(0, 150)}`);
            // If model is not found or unsupported, iterate to next candidate
            continue;
          }
        } catch (innerErr: any) {
          console.warn(`[Groq AI] Request error on ${groqModel}:`, innerErr?.message || innerErr);
          continue;
        }
      }
    } catch (groqErr: any) {
      console.warn('[Groq AI] Call timed out or failed, falling back to Gemini:', groqErr?.message || groqErr);
    }
  }

  // 6B. Try calling Gemini AI for intelligent contextual synthesis
...
`,
      ...
      // Use validated Gemini model (avoid non-gemini or deprecated models)
      const modelName = getValidGeminiModel(
        process.env.GEMINI_MODEL?.trim() ||
        (process.env.AI_MODEL?.trim()?.startsWith('gemini-') ? process.env.AI_MODEL.trim() : undefined)
      );
...
`;
      const aiRes = await ai.models.generateContent({
        model: getValidGeminiModel(process.env.GEMINI_MODEL || (process.env.AI_MODEL?.startsWith('gemini-') ? process.env.AI_MODEL : undefined)),
        contents: prompt,
      });
